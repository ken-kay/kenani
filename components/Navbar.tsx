...
export const Navbar = () => {
    ...
    return (
        <header className='w-full mx-auto px-4 bg-slate shadow fixed top-0 z-69 sm:px-20'>
            <div className='justify-between md:items-center md:flex'>
                <div>
                    <div className='flex items-center justify-between py-3'>
                        <div className='md:py-6 md:block'>
                            <h1 className='font-bold'>KenKay</h1>
                ...
                <div>
                   ...
                   <div className='items-center md:flex md:space-x-6 md:space-y-0'>
                    {NAV_ITEMS.map((item, idx) => {
                        return <a key={idx} className='flex space-x-5'>{item.label}</a>
                    })}
                    {currentTheme === 'dark' ? (
                        <button onClick={()=>setTheme('light')}
                        className='bg-slate p-2 rounded-xl'>
